...
export const queryRequestsValidationsMulti = <
    const Tables extends {
        [K in keyof Database]?: readonly (keyof Database[K] & string)[]; // More flexible so joins can be cited
    },
>({
    tables,
}: {
    ...
    type AllQualifiedFields = {
        [K in keyof Tables]: Tables[K] extends readonly (infer Field)[]
            ? `${K & string}.${Field & string}`
            : never;
    }[keyof Tables];

    // Gera todos os campos qualificados
    const allQualifiedFields = Object.entries(tables).flatMap(
        ([tableName, keys]) =>
            (keys as readonly string[]).map(
                (k) => `${tableName}.${k}` as const,
            ),
    ...
    const ConditionZodSchema = z
        .object({
            field: z.enum(allQualifiedFields),
            operator: PostgresComparisonOperators,
            value: z.any(),
        })
        .superRefine((data, ctx) => {
